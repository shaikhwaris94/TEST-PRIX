...
`
  ...
  summarizeShort = `Your task is to provide a concise summary of the changes and their architectural impact. This 
...
- **Architectural Summary**: Provide a high-level overview of the architectural changes (e.g., "Moves Auth logic from Controller to Middleware", "Refactors database access layer to use Repository pattern").
...
For code suggestions, use \`suggestion\` code blocks for one-click application on GitHub.
For fixes that aren't simple line replacements, use \`diff\` code blocks, marking changes with \`+\` or \`-\`.
...
- **Audit & Edge Cases**: Explicitly search for boundary condition failures (e.g., null pointers, off-by-one errors, empty collections, integer overflows).
- **Security**: Identify leaked API keys, SQL injection vulnerabilities, and other security flaws.
- **Performance & Cost**: Spot O(n^2) loops, redundant calculations, and inefficient database queries. Explain the potential infrastructure cost implications for high-traffic paths.
- **Architecture**: Flag violations of clean architecture principles (e.g., business logic depending on infrastructure/framework details).
- **Tests**: If new logic is introduced without corresponding tests, generate a \`jest\` or \`vitest\` test snippet that covers the new logic and edge cases.
- **Documentation**: Warn if a function signature changed but JSDoc/docstrings were not updated.
- **Contextual Replay**: If a change breaks an undocumented invariant or contract in another file (refer to the "Usage Context" if provided), call it out as a high-severity issue.

### Confidence Score:
For every suggestion, provide a **Confidence Score** (0-100%). If your confidence is below 80%, explain why (e.g., "Missing context on library X").
...
`
  ...
  fixSuggestion = `The following code suggestion for \`$filename\` failed CI validation:
...
Please provide a corrected version of the suggestion that fixes the reported issues. 
Ensure the suggestion is still within the same line range and follows the same format.
Respond ONLY with the corrected suggestion block.
`

  renderFixSuggestion(inputs: Inputs, suggestion: string, error: string): string {
    const ins = inputs.clone()
    ins.suggestion = suggestion
    ins.error = error
    return ins.render(this.fixSuggestion)
  }
}
